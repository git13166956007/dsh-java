...
| Sub-agents | Persisted sub-agent profiles and continuable sessions with independent model/mode/prompt, tool and Skill allowlists, priority, cost weight, capability tags and concurrency limits; plan steps and active Agents can dispatch enabled execution workers; HTTP API can start, inspect, stream and cancel workers; persisted async Sub-agent Runs are reattached after restart without persisting request API Keys; adaptive candidate scoring and diagnostics are exposed | Learned routing and cross-process distributed quotas |
