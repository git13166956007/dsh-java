...
| Sub-agents | Persisted sub-agent profiles and continuable sessions with independent model/mode/prompt, tool and Skill allowlists; plan steps and active Agents can dispatch enabled execution workers; HTTP API can start, inspect, stream and cancel workers | Reattach in-flight workers after restart, richer adaptive delegation policies and cost-aware routing |
