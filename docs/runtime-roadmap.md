...
| MCP | Java SDK client with stdio/SSE/Streamable HTTP, durable profiles, encrypted credentials, startup reconnect with backoff, initialize/list-tools/call-tool, resource/prompt inspection, lifecycle API, Server-level approval policy, persistent health telemetry and ToolRegistry synchronization | Resource subscriptions and richer transport diagnostics |
| Skills | Filesystem `skills/<name>/SKILL.md`, front matter with version, resource indexing, durable enable/disable state and system-prompt injection | Package installation/update management |
| Models | Persisted profiles with active selection, built-in and plugin-registered Provider routing, encrypted API keys, proxy/request parameters, capability flags, context budgets, Provider extension JSON, connectivity tests, policy-driven fallback routing and durable health counters; Provider tokenizer hook is wired into context budgeting | Exact tokenizer implementations for individual providers |
...
| Sub-agents | Persisted sub-agent profiles with independent model/mode/prompt, tool and Skill allowlists; plan steps and active Agents can dispatch enabled execution workers; HTTP API can start, inspect, stream and cancel background workers | Persistent worker recovery after restart, richer adaptive delegation policies and cost-aware routing |
| Plans | Persisted Plan/PlanStep state, approval, generated/manual dependencies, dependency-aware asynchronous execution, bounded parallelism, retries, cancellation API and durable parent-child run IDs | Richer dependency policies |
| Context | Conversation IDs, persisted messages, configurable message/token budgets, newest-first truncation, rolling model-generated summaries, Provider tokenizer hooks and context inspection/compaction APIs | System/context providers and exact provider tokenizer implementations |
...
Active Agents can now call `delegate_to_subagent` when enabled execution profiles exist. The call is recorded as a normal tool event and creates a child Run when Run persistence is enabled; the parent and child depth budgets are enforced. If a child tool requires approval, the parent delegation pauses and approval resumes the child before continuing the parent. The Sub-agent API can also start an independent execution worker, stream its persisted and live events, and cancel its in-process model call; persisted Run records are not yet reattached to live workers after an application restart.
